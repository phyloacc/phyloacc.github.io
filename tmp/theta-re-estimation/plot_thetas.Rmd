...
3. I recovered the theta values $\theta$ used as input for the simulations from the file `Yan-etal-2022/data/SimulatedData/neut_ver3_genetree2.named.mod`, as the number following the `#` symbol in the branch lengths in the provided tree:
...
tree_info_long = sim_tree_info %>% 
       ...
       select(label, bl.subs, bl.coal, sim.theta, est.theta) %>% 
       pivot_longer(cols=c("est.theta", "sim.theta"), names_to="theta.cat", values_to="theta")
...
sim_data %>% 
  ...
  rename(Branch=label, Branch.length.subs=bl.subs, Branch.length.coalescent=bl.coal, Estimated.theta=est.theta, Input.theta=sim.theta, Theta.diff=theta.diff) %>%
  kable() %>% 
